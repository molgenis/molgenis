  ...
  <div v-if="acls.length">
    <table class="table table-sm table-responsive">
      ...
      <tr v-for="(acl, rowIndex) in acls">
        <td>
          <router-link :to="linkToResource(rowIndex)">{{acl.entityLabel || acl.entityId}}</router-link>
        </td>
        <td>{{acl.owner}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      linkToResource (rowIndex) {
        const acl = this.rows[rowIndex]
        const entityTypeId = this.selectedEntityTypeId
        const entityId = acl.entity
        return `/resource/${entityTypeId}/${entityId}`
      }
    },
    computed: {
      ...mapState(['rows', 'selectedEntityTypeId'])
    },
    filters: {
