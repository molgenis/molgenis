...
public class Step8VarcharToTextRepeated extends MolgenisUpgrade
{
				...
				if (fieldType instanceof StringField)
				{
							...
							|| fieldType instanceof CategoricalField || fieldType instanceof MrefField || fieldType instanceof XrefField))
					{
						...
						if (amd.isUnique())
						{
							...
							try
							{
								template.execute(getRemoveUniqueConstraintSql(emd, amd));
							}
							catch (DataAccessException dae)
							{
								LOG.error("Error removing UNIQUE constraint for {}.{}.{} ", mysql.getName(),
										emd.getName(), amd.getName(), dae);
							}
						}
	...
	private static String getRemoveUniqueConstraintSql(EntityMetaData emd, AttributeMetaData att)
	{
		return String.format("ALTER TABLE %s DROP INDEX %s;", emd.getName(), att.getName());
	}
}
