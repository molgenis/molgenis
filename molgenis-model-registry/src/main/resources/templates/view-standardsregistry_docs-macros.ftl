...
<#macro renderAttribute attribute entity depth>
	...
	<tr id="attribute-${entity.name?replace(" ", "_")?html}${attribute.name?replace(" ", "_")?html}">
    	...
    	<td>${dataType?html}<#if dataType == "CATEGORICAL" || dataType == "MREF" || dataType == "XREF"> (<a href="#entity-${attribute.refEntity.name?replace(" ", "_")?html}">${attribute.refEntity.label?html}</a>)</#if></td>
    	<td>
    	    <#assign constraints = []>
