...
public class MappingServiceController extends MolgenisPluginController
{
	...
	public @ResponseBody Map<String, String> addTagAttribute(@Valid @RequestBody AddTagRequest request)
	{
		...
		return getLabelIriMap(request.getOntologyTermIRIs());
	}
	...
	public @ResponseBody Map<String, Map<String, String>> autoTagAttributes(@Valid @RequestBody AutoTagRequest request)
	{
		...
		addAutoGeneratedTagsToRepository(request, autoGeneratedTags);
		return getAttributeTagLabelIriMap(autoGeneratedTags);
	}
	...
	private Map<String, String> getLabelIriMap(List<String> IRIs)
	{
		Map<String, String> labelIriMap = new HashMap<String, String>();
		if (IRIs.size() < 2 && !IRIs.isEmpty())
		{
	...
	private void addAutoGeneratedTagsToRepository(AutoTagRequest request,
			Map<AttributeMetaData, List<OntologyTerm>> autoGeneratedTags)
	{
		for (AttributeMetaData amd : autoGeneratedTags.keySet())
		{
			if (!autoGeneratedTags.get(amd).isEmpty())
			{
				List<String> ontologyTermIRIs = new ArrayList<String>();
				for (OntologyTerm ontologyTerm : autoGeneratedTags.get(amd))
				{
					ontologyTermIRIs.add(ontologyTerm.getIRI());
				}
				ontologyTagService.addAttributeTag(request.getEntityName(), amd.getName(),
						Relation.isAssociatedWith.getIRI(), ontologyTermIRIs);
			}
		}
	}

	private Map<String, Map<String, String>> getAttributeTagLabelIriMap(
			Map<AttributeMetaData, List<OntologyTerm>> autoGeneratedTags)
	{
		Map<String, Map<String, String>> attributeTagLabelIriMap = new HashMap<String, Map<String, String>>();
		for (AttributeMetaData amd : autoGeneratedTags.keySet())
		{
			List<String> ontologyTermIRIs = new ArrayList<String>();
			for (OntologyTerm ontologyTerm : autoGeneratedTags.get(amd))
			...
			attributeTagLabelIriMap.put(amd.getName(), getLabelIriMap(ontologyTermIRIs));
		}

		return attributeTagLabelIriMap;
	}
