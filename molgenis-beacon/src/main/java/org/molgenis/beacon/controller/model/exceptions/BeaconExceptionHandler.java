...
/**
 * Handles only the {@link BeaconException} that has to be converted to {@link BeaconAlleleResponse}
 * to conform to Beacon network specifications
 */
@ControllerAdvice
...
public class BeaconExceptionHandler {
  ...
  public BeaconAlleleResponse handleUnknownBeaconException(UnknownBeaconException e) {
    ...
    return BeaconAlleleResponse.create(
        e.getBeaconId(),
        null,
        BeaconError.create(HttpStatus.BAD_REQUEST.value(), e.getMessage()),
        e.getRequest());
  }
