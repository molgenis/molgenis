	...
	function createResults(entities){
		...
		$.each(entities, function(rowIndex, entity){
			...
			$.each(entity.results.searchHits, function(index, hit){
				...
				var ontologyTermName = $('<div />').addClass('span5 matchterm show-popover').css('margin-bottom', '-6px').append(hit.columnValueMap.ontologyTerm);
				var ontologyTermUrl = $('<div />').addClass('span5 termurl').css('margin-bottom', '-6px').append('<a href="' + hit.columnValueMap.ontologyTermIRI + '" target="_blank">' + hit.columnValueMap.ontologyTermIRI + '</a>');
