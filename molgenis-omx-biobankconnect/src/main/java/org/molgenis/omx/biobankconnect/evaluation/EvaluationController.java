						...
						if (ds.getName().equalsIgnoreCase(firstColumn))
						{
								...
								if (feature != null)
								{
									...
									for (DataSet dataSet : dataSets)
									{
										...
										if (mappingDetail.containsKey(dataSet.getName().toLowerCase()))
										{
											...
											{
												Map<String, Object> columnValueMap = hit.getColumnValueMap();
												String mappedFeatureId = columnValueMap.get(
														"store_mapping_mapped_feature").toString();
												...
												if (previousDescription != null
														&& previousDescription
																.equalsIgnoreCase(mappedFeatureDescription)) rank--;
										...
										rankCollection.get(description).put(dataSet.getName().toLowerCase(),
												ranksBiobank);
									}
							...
							Map<String, List<Integer>> rankCollectionPerBiobank = new HashMap<String, List<Integer>>();
							{
								sheetWriterRankStatistics = excelWriterRanks.createWritable("rank statistics", Arrays
										.asList(firstColumn.toLowerCase(), "average rank", "round-up rank",
												"median rank", "minium", "maximum"));
								...
								for (Entry<String, Map<String, List<Integer>>> entry : rankCollection.entrySet())
								{
									...
									for (Entry<String, List<Integer>> rankBiobanks : entry.getValue().entrySet())
									{
										...
										rankCollectionPerBiobank.get(rankBiobanks.getKey()).addAll(
												rankBiobanks.getValue());
							...
							{
								sheetWriteSpssInput = excelWriterRanks.createWritable("spss ranks",
										Arrays.asList("rank"));
