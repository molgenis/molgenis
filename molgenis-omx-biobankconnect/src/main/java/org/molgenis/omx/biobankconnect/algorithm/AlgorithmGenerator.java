...
public class AlgorithmGenerator
{
	...
	public void applyAlgorithm(String userName, Integer targetDataSetId, List<Integer> sourceDataSetIds)
	{
		...
		{
			findAllAlgorithms(userName, query, targetDataSetId, sourceDataSetId);
		}
		dataService.getCrudRepository(ObservedValue.ENTITY_NAME).flush();
		currentUserStatus.setUserIsRunning(userName, false);
	}

	private void findAllAlgorithms(String userName, QueryImpl query, Integer targetDataSetId, Integer sourceDataSetId)
	{
		String dataSetIdentifier = createMappingDataSetIdentifier(userName, targetDataSetId, sourceDataSetId);
		DataSet dataSet = dataService.findOne(DataSet.ENTITY_NAME,
				new QueryImpl().eq(DataSet.IDENTIFIER, dataSetIdentifier), DataSet.class);
		SearchResult searchResult = searchService.search(new SearchRequest(dataSetIdentifier, query, null));
		...
		for (Hit hit : searchResult.getSearchHits())
		{
			...
			for (Entry<Integer, Object> entry : createValueFromAlgorithm(sourceDataSetId, algorithmScript).entrySet())
			{
	...
	private void createDerivedDataSets(String userName, Integer targetDataSetId, List<Integer> sourceDataSetIds)
	{
		...
		Iterable<DataSet> sourceDataSets = dataService.findAll(DataSet.ENTITY_NAME,
				new QueryImpl().in(DataSet.ID, sourceDataSetIds), DataSet.class);
		for (DataSet sourceDataSet : sourceDataSets)
		{
			DataSet dataSet = new DataSet();
			dataSet.setIdentifier(createDerivedDataSetIdentifier(userName, targetDataSet, sourceDataSet));
			dataSet.setName(createDerivedDataSetName(userName, targetDataSet, sourceDataSet));
			dataSet.setProtocolUsed(targetDataSet.getProtocolUsed());
			dataService.add(DataSet.ENTITY_NAME, dataSet);
		}
		dataService.getCrudRepository(DataSet.ENTITY_NAME).flush();
	}

	private String createDerivedDataSetIdentifier(String userName, DataSet targetDataSet, DataSet sourceDataSet)
	{
		StringBuilder dataSetIdentifier = new StringBuilder();
		dataSetIdentifier.append("derived-").append(userName).append('-').append(targetDataSet.getId()).append('-')
				.append(sourceDataSet.getId());
		return dataSetIdentifier.toString();
	}

	private String createDerivedDataSetName(String userName, DataSet targetDataSet, DataSet sourceDataSet)
	{
		StringBuilder dataSetIdentifier = new StringBuilder();
		dataSetIdentifier.append(userName).append(": derived ").append(targetDataSet.getName()).append(" in ")
				.append(sourceDataSet.getName());
		return dataSetIdentifier.toString();
	}
