...
public class GroupPermissionServiceTest extends AbstractMockitoTest
{
	...
	public void testGrantPermissions()
	{
		...
		verify(permissionService).grant(new PackageIdentity("bbmri_eric"), WRITEMETA,
				createRoleSid("BBMRI_ERIC_MANAGER"));
		verify(aclService).createAcl(new GroupIdentity("bbmri-eric"));
		verify(permissionService).grant(new GroupIdentity("bbmri-eric"), WRITEMETA,
				createRoleSid("BBMRI_ERIC_MANAGER"));
	}
