...
public class AttributeRepositorySecurityDecoratorTest extends AbstractMockitoTestNGSpringContextTests
{
	...
	public void countUser() throws Exception
	{
		...
		when(permissionService.hasPermission(EntityTypeIdentity.TYPE, entityType0Name,
				EntityTypePermission.COUNT)).thenReturn(false);
		when(permissionService.hasPermission(EntityTypeIdentity.TYPE, entityType1Name,
	...
	public void forEachBatchedUser() throws Exception
	{
		...
		when(permissionService.hasPermission(EntityTypeIdentity.TYPE, "EntityType1",
				EntityTypePermission.COUNT)).thenReturn(true);
		when(permissionService.hasPermission(EntityTypeIdentity.TYPE, "EntityType2",
				EntityTypePermission.COUNT)).thenReturn(false);
		when(permissionService.hasPermission(EntityTypeIdentity.TYPE, "EntityType3",
				EntityTypePermission.COUNT)).thenReturn(false);
		when(permissionService.hasPermission(EntityTypeIdentity.TYPE, "EntityType4",
				EntityTypePermission.COUNT)).thenReturn(true);
