...
public class PackagePermissionUtilsTest extends AbstractMockitoTest
{
	...
	public void testIsWritablePackageAddPackagePermission()
	{
		...
		doReturn(true).when(userPermissionEvaluator).hasPermission(new PackageIdentity("packageId"), ADD_PACKAGE);
		doReturn(false).when(userPermissionEvaluator).hasPermission(new PackageIdentity("packageId"), ADD_ENTITY_TYPE);
	...
	{
		Package aPackage = when(mock(Package.class).getId()).thenReturn("packageId").getMock();
		doReturn(true).when(userPermissionEvaluator).hasPermission(new PackageIdentity("packageId"), ADD_ENTITY_TYPE);
		assertTrue(PackagePermissionUtils.isWritablePackage(aPackage, userPermissionEvaluator));
	}
