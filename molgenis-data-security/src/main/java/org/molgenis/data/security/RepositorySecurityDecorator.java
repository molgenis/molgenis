...
/**
 * Repository decorated that validates that current user has permission to perform an operation on
 * an entity type's data.
 */
public class RepositorySecurityDecorator extends AbstractRepositoryDecorator<Entity> {
  private final UserPermissionEvaluator permissionService;

  public RepositorySecurityDecorator(
      Repository<Entity> delegateRepository, UserPermissionEvaluator permissionService) {
  ...
  private void validatePermission(EntityType entityType, EntityTypePermission permission) {
    boolean hasPermission =
        permissionService.hasPermission(new EntityTypeIdentity(entityType.getId()), permission);
    if (!hasPermission) {
