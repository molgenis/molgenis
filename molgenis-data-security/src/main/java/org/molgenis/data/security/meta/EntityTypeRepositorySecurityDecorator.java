...
public class EntityTypeRepositorySecurityDecorator
    extends AbstractRowLevelSecurityRepositoryDecorator<EntityType> {
  ...
  private boolean checkEntityTypePermission(String entityTypeId, Action action) {
    ...
    boolean hasPermission =
        userPermissionEvaluator.hasPermission(new EntityTypeIdentity(entityTypeId), permission);
  ...
  public void updateAcl(EntityType entityType) {
    MutableAcl acl =
        (MutableAcl) mutableAclService.readAclById(new EntityTypeIdentity(entityType.getId()));
    Package pack = entityType.getPackage();
    if (pack != null) {
      ObjectIdentity objectIdentity = new PackageIdentity(pack);
