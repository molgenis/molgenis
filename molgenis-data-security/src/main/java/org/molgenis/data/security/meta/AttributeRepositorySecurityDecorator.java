...
public class AttributeRepositorySecurityDecorator extends AbstractRepositoryDecorator<Attribute> {
  ...
  public void delete(Stream<Attribute> attrs) {
    // The validateDeleteAllowed check if querying the table in which we are deleting. Since the
    // decorated repo only
    // guarantees that the attributes are deleted after the operation completes we have to delete
    // the attributes one
    // by one
  ...
  private Attribute filterReadPermission(Attribute attr) {
    return attr != null
        ? filterReadMetadataPermission(Stream.of(attr)).findFirst().orElse(null)
        : null;
  }
  ...
  private class FilteredConsumer {
    ...
    public void filter(List<Attribute> attrs) {
      Stream<Attribute> filteredAttrs =
          filterPermission(attrs.stream(), EntityTypePermission.READ_METADATA);
