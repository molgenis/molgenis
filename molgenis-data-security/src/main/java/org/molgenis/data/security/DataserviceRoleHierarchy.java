...
public class DataserviceRoleHierarchy implements RoleHierarchy
{
			...
			Collection<? extends GrantedAuthority> authorities)
	{
		...
		{
			Set<String> included = newlyDiscovered.stream()
		...
		Set<SimpleGrantedAuthority> reachableRoles = roleNames.stream()
															  .map(SidUtils::createRoleAuthority)
															  .map(SimpleGrantedAuthority::new)
															  .collect(toSet());
		LOG.debug("getReachableGrantedAuthorities() - From the roles {} one can reach {} in zero or more steps.",
