(function($, molgenis) {
	...
	molgenis.createFeatureSelection = function(protocolUri) {
		...
		function onNodeSelectionChange(selectedNodes) {
			...
			var sortedFeatures = $.map(sortedNodes, function(node) {
				return node.data.isFolder || node.data.key == 'more' ? null
						: node.data.key;
			});
							...
							container
									.dynatree({
										...
										onLazyRead : function(node) {
											// workaround for dynatree lazy
											// parent node select bug
						...
						function(feature) {
							...
							case "string":
								if (config == null) {
									...
									filter = $('<input type="text" placeholder="filter text" autofocus="autofocus" value="'
											+ config.values[0] + '">');
								}
								...
								if (config == null)
									...
									filterTo = datePickerTo
											.append($('<input id="date-feature-to" type="text" value="'
													+ config.values[1].replace(
															"T", "'T'")
													+ '"><span class="add-on"><i data-time-icon="icon-time" data-date-icon="icon-calendar"></i></span>'));

								filter = $('<span>From:<span>').after(
										filterFrom)
										.after($('<span>To:</span>')).after(
												filterTo);
								$(".feature-filter-dialog").dialog("option",
										"width", 710);
							...
							case "decimal":
								...
								if (config == null) {
									...
									fromFilter = $('<input id="from" type="number" autofocus="autofocus" step="any" value="'
											+ config.values[0] + '">');
									toFilter = $('<input id="to" type="number" autofocus="autofocus" step="any" value="'
											+ config.values[1] + '">');
								}

								fromFilter.on('keyup input', function() {
									// If 'from' changed set 'to' at the same
									// value
									$('#to').val($('#from').val());
								});
							...
							case "bool":
								...
								$('input[name=bool-feature]').live('change',
										function() {
											applyButton.removeAttr('disabled');
										});
											...
											molgenis
													.updateFeatureFilter(
															featureUri,
															{
																...
																type : feature.dataType,
																values : [ $(
																		'input[name=bool-feature]:checked')
																		.val() ]
															});
										});
								break;
							case "categorical":
																	...
																	if (config
																			&& ($
																					.inArray(
																							this.name,
																							config.values) > -1)) {
																		input = $('<input type="checkbox" class="cat-value" name="'
																				+ feature.identifier
																				+ '" value="'
																				+ this.name
																				+ '" checked>');
								...
								break;
							default:
								console.log("TODO: '" + feature.dataType
										+ "' not supported");
								return;
							}

							var applyButtonHolder = $(
									'<div id="applybutton-holder" />').append(
									applyButton);
							...
							$('.feature-filter-dialog').html(items.join(''))
									.append(filter);

							if ((feature.dataType == 'xref')
									|| (feature.dataType == 'mref')) {
								$('.feature-filter-dialog input[type=text]')
										.autocomplete(
												{
	...
	molgenis.download = function() {
		...
		parent.showSpinner();
		$.download('/plugin/dataexplorer/download', {
