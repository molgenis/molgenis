...
{
	public GroupValue createGroup(String groupName, String label, @Nullable String description,
			boolean publiclyVisible, Collection<String> roleLabels)
	{
		...
		roleLabels.stream().map(roleLabel -> createRoleValue(groupName, label, roleLabel)).forEach(rolesBuilder::add);
