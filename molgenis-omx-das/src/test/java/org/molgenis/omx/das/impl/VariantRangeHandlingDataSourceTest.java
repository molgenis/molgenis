...
public class VariantRangeHandlingDataSourceTest
{
	...
	public void setUp() throws HandleRequestDelegationException, Exception
	{
		...
		expectedEntryPoint = new DasEntryPoint("Chromosome_name", new Integer(0), new Integer(48000000), "Chromosome",
		...
		when(
				dataService.findOne(eq(Chromosome.ENTITY_NAME), any(Query.class),
						Matchers.notNull(Chromosome.class.getClass()))).thenReturn(chromosome);
		when(dataService.findOne(eq(Track.ENTITY_NAME), any(Query.class), Matchers.notNull(Track.class.getClass())))
				.thenReturn(track);
	}
			...
			BadReferenceObjectException, CoordinateErrorException
	{
		...
		verify(dataService).findAllAsList(
				Variant.ENTITY_NAME,
				new QueryImpl().ge(Variant.BPSTART, 1).le(Variant.BPEND, 100000).eq(Variant.CHROMOSOME, chromosome)
						.eq(Variant.TRACK, track));
	}

	@Test()
	public void getFeaturesRangePatientOneAllele() throws UnimplementedFeatureException, DataSourceException,
			BadReferenceObjectException, CoordinateErrorException
	{

		when(patient.getAllele1()).thenReturn(variant);
		when(patient.getAllele2()).thenReturn(null);
		when(dataService.findOne(eq(Patient.ENTITY_NAME), any(Query.class), Matchers.notNull(Patient.class.getClass())))
				.thenReturn(patient);
		...
		verify(dataService).findAllAsList(
				Variant.ENTITY_NAME,
				new QueryImpl().ge(Variant.BPSTART, 1).le(Variant.BPEND, 100000).eq(Variant.CHROMOSOME, chromosome)
						.eq(Variant.TRACK, track).eq(Variant.IDENTIFIER, "vatiant_identifier"));
	}

	@Test()
	public void getFeaturesRangePatientTwoAlleles() throws UnimplementedFeatureException, DataSourceException,
			BadReferenceObjectException, CoordinateErrorException
	{
		when(patient.getAllele1()).thenReturn(variant);
		...
		verify(dataService).findAllAsList(
				Variant.ENTITY_NAME,
				new QueryImpl().ge(Variant.BPSTART, 1).le(Variant.BPEND, 100000).eq(Variant.CHROMOSOME, chromosome)
						.eq(Variant.TRACK, track).eq(Variant.IDENTIFIER, "vatiant_identifier").or()
						.eq(Variant.IDENTIFIER, "vatiant_identifier2"));
	}

	@Test()
	public void getFeaturesRangePatientNoAlleles() throws UnimplementedFeatureException, DataSourceException,
			BadReferenceObjectException, CoordinateErrorException
	{
	...
	@Test()
	public void getTotalCountForType() throws UnimplementedFeatureException, DataSourceException,
			BadReferenceObjectException, CoordinateErrorException
	{
