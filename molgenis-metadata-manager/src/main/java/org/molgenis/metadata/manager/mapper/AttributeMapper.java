...
public class AttributeMapper {
  ...
  private EditorAttribute toEditorAttribute(Attribute attribute) {
    ...
    String type =
        attribute.getDataType() != null
            ? AttributeType.getValueString(attribute.getDataType())
            : null;
  ...
  private ImmutableMap<String, String> toI18nLabel(Attribute attribute) {
    ...
    getLanguageCodes()
        .forEach(
            languageCode -> {
              // attribute.getLabel cannot be used, since it returns the description in the default
  ...
  private ImmutableMap<String, String> toI18nDescription(Attribute attribute) {
    ...
    getLanguageCodes()
        .forEach(
            languageCode -> {
              // attribute.getDescription cannot be used, since it returns the description in the
