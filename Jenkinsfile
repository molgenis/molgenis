                ...
                stage('Build [ PR ]') {
                    steps {
                        container('maven') {
                            ...
                            sh "mvn -q -B sonar:sonar -Dsonar.analysis.mode=preview -Dsonar.login=${env.SONAR_TOKEN} -Dsonar.github.oauth=${env.GITHUB_TOKEN} -Dsonar.github.pullRequest=${env.CHANGE_ID} -Dsonar.ws.timeout=120 -Dsonar.projectKey=org.molgenis:molgenis:PR"
                            dir('molgenis-app'){
