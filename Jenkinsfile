                ...
                stage('Build [ PR ]') {
                    steps {
                        container('maven') {
                            ...
                            sh "mvn -q -B sonar:sonar -Dsonar.login=${env.SONAR_TOKEN} -Dsonar.github.oauth=${env.GITHUB_TOKEN} -Dsonar.projectKey=org.molgenis:molgenis:master -Dsonar.pullrequest.base=master -Dsonar.pullrequest.branch=${BRANCH_NAME} -Dsonar.pullrequest.key=${env.CHANGE_ID} -Dsonar.pullrequest.provider=GitHub -Dsonar.pullrequest.github.repository=molgenis/molgenis -Dsonar.ws.timeout=120"
                            dir('molgenis-app'){
