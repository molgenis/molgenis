...
public class SortaServiceImplTest
{
	...
	public void beforeClass()
	{
		...
		sortaService = new SortaServiceImpl(dataService, mock(InformationContentService.class));
		...
		// DataService action for matching ontologyterm synonyms
		QueryRule disMaxQueryRule = new QueryRule(Arrays.asList(new QueryRule(
		...
		when(dataService.findAll(OntologyTermMetaData.ENTITY_NAME, new QueryImpl(finalQueryRules).pageSize(100)))
		...
		// DataService action for matching ontologyterm synonyms
		QueryRule disMaxQueryRule_2 = new QueryRule(Arrays.asList(new QueryRule(
		...
		when(
				dataService.findAll(
						OntologyTermMetaData.ENTITY_NAME,
						new QueryImpl(Arrays.asList(new QueryRule(OntologyTermMetaData.ONTOLOGY, Operator.EQUALS,
								ontologyEntity), new QueryRule(Operator.AND), disMaxQueryRule_2)).pageSize(99)))
		...
		// DataService action for matching ontologyterm synonyms
		QueryRule disMaxQueryRule_3 = new QueryRule(Arrays.asList(new QueryRule(
				OntologyTermMetaData.ONTOLOGY_TERM_SYNONYM, Operator.FUZZY_MATCH,
				"proptosi~0.8 protrud~0.8 ey~0.8 exophthalmo~0.8")));
		disMaxQueryRule_3.setOperator(Operator.DIS_MAX);

		when(
				dataService.findAll(
						OntologyTermMetaData.ENTITY_NAME,
						new QueryImpl(Arrays.asList(new QueryRule(OntologyTermMetaData.ONTOLOGY, Operator.EQUALS,
								ontologyEntity), new QueryRule(Operator.AND), disMaxQueryRule_3)).pageSize(100)))
