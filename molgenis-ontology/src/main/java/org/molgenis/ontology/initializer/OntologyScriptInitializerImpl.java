...
public class OntologyScriptInitializerImpl implements OntologyScriptInitializer
{
	...
	public void initialize()
	{
		...
		if (resource.exists())
		{
			...
			if (count == 0)
			{
				String scriptTypeId = "R";
				Entity scriptType = dataService.findOne(SCRIPT_TYPE,
						new QueryImpl<>().eq(ScriptTypeMetaData.NAME, scriptTypeId));

				if (scriptType == null)
				{
					throw new UnknownEntityException(dataService.getEntityType(SCRIPT_TYPE), scriptTypeId);
				}
