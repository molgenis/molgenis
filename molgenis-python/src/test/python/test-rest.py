...
class TestStringMethods(unittest.TestCase):
  ...
  def test_login_and_get_superuser_username(self):
    ...
    self.assertEquals('admin',response[0]['username'])
  ...
  def test_login_logout_and_get_MolgenisUser(self):
    ...
    try:
      ...
      self.assertEquals(e.response.status_code, 401)
      self.assertEquals(e.response.json()['errors'][0]['message'], 'No READ permission on entity MolgenisUser')
  ...
  def test_add_all(self):
    ...
    self.assertEquals(['ref55','ref57'], response)
    item55 = s.get('TypeTestRef', q=[{"field":"value", "operator":"EQUALS", "value":"ref55"}])[0]
    self.assertItemsEqual({"value": "ref55", "label": "label55", "href": "/api/v1/TypeTestRef/ref55"}, item55)
    s.delete('TypeTestRef', 'ref55')
    s.delete('TypeTestRef', 'ref57')
  ...
  def test_add_dict(self):
    ...
    try:
      ...
      print(str(e))
    self.assertEquals('ref55', s.add('TypeTestRef',{"value": "ref55","label": "label55"}))
    s.delete('TypeTestRef', 'ref55')
