...
class Session():
	...
	def login(self, username, password):
		...
		'''
		response = requests.post(self.url + "v1/login", 
	...
	def logout(self):
		...
		response = requests.post(self.url + "v1/logout",
	...
	def get(self, entity, q=None, attributes=None, num = 1000, start = 0, sortColumn= None, sortOrder = None):
		...
		'''
		if q:
			response = requests.post(self.url + "v1/" + urllib.quote_plus(entity),
				headers = self._get_token_header_with_content_type(),
				params={"_method":"GET", "attributes":attributes, "num": num, "start": start, "sortColumn":sortColumn, "sortOrder": sortOrder},
				data=json.dumps({"q":q}))
		else:
			response = requests.get(self.url + "v1/" + urllib.quote_plus(entity),
				headers = self._get_token_header(),
	...
	def add_all(self, entity, entities):
		...
		response = requests.post(self.url+"v2/"+urllib.quote_plus(entity),
			headers = self._get_token_header_with_content_type(),
	...
	def delete(self, entity, id):
		...
		response = requests.delete(self.url + "v1/" + urllib.quote_plus(entity)+ "/" + urllib.quote_plus(id), headers = self._get_token_header())
		response.raise_for_status();
		return response;

	def get_entity_meta_data(self, entity):
		'''Retrieves the metadata for an entity repository.'''
		response = requests.get(self.url + "v1/" + urllib.quote_plus(entity) + "/meta?expand=attributes", headers = self._get_token_header()).json()
		response.raise_for_status();
		return response;

	def get_attribute_meta_data(self, entity, attribute):
		'''Retrieves the metadata for a single attribute of an entity repository.'''
		response = requests.get(self.url + "v1/" + urllib.quote_plus(entity) + "/meta/"+ urllib.quote_plus(attribute), headers = self._get_token_header()).json()
		response.raise_for_status();
		return response;
