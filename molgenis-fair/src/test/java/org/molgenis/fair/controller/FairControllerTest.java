...
public class FairControllerTest extends AbstractTestNGSpringContextTests {
  ...
  public void getCatalogTestForwarded() throws Exception {
    ...
    this.mockMvc
        .perform(
            get("/api/fdp/catalogID")
                .header("X-Forwarded-Host", "website.com")
                .contentType(MediaType.APPLICATION_FORM_URLENCODED))
        .andExpect(status().isOk());

    Mockito.verify(entityModelWriter)
        .createRdfModel("http://website.com/api/fdp/catalogID", answer);
  }
  ...
  public void getDatasetTest() throws Exception {
    ...
    this.mockMvc
        .perform(
            get("/api/fdp/catalogID/datasetID")
                .header("X-Forwarded-Host", "website.com")
                .contentType(MediaType.APPLICATION_FORM_URLENCODED))
        .andExpect(status().isOk());

    Mockito.verify(entityModelWriter)
        .createRdfModel("http://website.com/api/fdp/catalogID/datasetID", answer);
  }
  ...
  public void getDistributionTest() throws Exception {
    ...
    this.mockMvc
        .perform(
            get("/api/fdp/catalogID/datasetID/distributionID")
                .header("X-Forwarded-Host", "website.com")
