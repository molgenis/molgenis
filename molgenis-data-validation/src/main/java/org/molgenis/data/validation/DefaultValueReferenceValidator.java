...
public interface DefaultValueReferenceValidator {
  ...
  /**
   * Appends a filter to the given stream that throws a MolgenisValidationException if an entity is
   * referenced by an Attribute.
   */
  Stream<Entity> validateEntitiesNotReferenced(Stream<Entity> entityStream, EntityType entityType);

  /**
   * @throws MolgenisValidationException if the entity with the given id is referenced by an
   *     Attribute
   */
  void validateEntityNotReferencedById(Object entityId, EntityType entityType);

  /**
   * Appends a filter to the given stream that throws a MolgenisValidationException if an entity
   * with the given id is referenced by an Attribute.
   */
