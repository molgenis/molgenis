...
public class EntityTypeValidator {
  ...
  /**
   * Validates that the label attribute of this EntityType: 1) Is visible. 2) Is not null if the id
   * attribute is hidden.
   *
    ...
    if (entityType.getLabelAttribute() != null && !entityType.getLabelAttribute().isVisible()) {
      throw new MolgenisValidationException(
          new ConstraintViolation(
              format(
                  "Label attribute [%s] of EntityType [%s] must be visible.",
                  entityType.getLabelAttribute().getName(), entityType.getId())));
    }

    if (!entityType.isAbstract()
        && !entityType.getIdAttribute().isVisible()
        && (entityType.getLabelAttribute() == null)) {
      throw new MolgenisValidationException(
          new ConstraintViolation(
              format(
                  "EntityType [%s] must define a label attribute because the identifier is hidden",
                  entityType.getId())));
    }
  }
   ...
   */
  static void validateOwnLookupAttributes(
            ...
            ownLookupAttr -> {
              ...
              if (ownAttr == null) {
                throw new MolgenisValidationException(
                    new ConstraintViolation(
                        format(
                            "Lookup attribute [%s] is not one of the attributes of entity [%s]",
              ...
              if (!ownLookupAttr.isVisible()) {
                throw new MolgenisValidationException(
                    new ConstraintViolation(
                        format(
                            "Lookup attribute [%s] of entity type [%s] must be visible",
                            ownLookupAttr.getName(), entityType.getId())));
              }
            });
  }
   ...
   */
  static void validateOwnLabelAttribute(
      EntityType entityType, Map<String, Attribute> ownAllAttrMap) {
    ...
    if (ownLabelAttr != null) {
      ...
      if (ownAttr == null) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "Label attribute [%s] is not one of the attributes of entity [%s]",
      ...
      if (ownAttr.isNillable()) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "Label attribute [%s] of entity type [%s] cannot be nillable",
                    ownLabelAttr.getName(), entityType.getId())));
      }
    }
  }
    ...
    if (ownIdAttr != null) {
      ...
      if (ownAttr == null) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "EntityType [%s] ID attribute [%s] is not part of the entity attributes",
      ...
      if (!AttributeUtils.isIdAttributeTypeAllowed(ownIdAttr)) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "EntityType [%s] ID attribute [%s] type [%s] is not allowed",
      ...
      if (!ownIdAttr.isUnique()) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "EntityType [%s] ID attribute [%s] is not a unique attribute",
                    entityType.getId(), ownIdAttr.getName())));
      }
      ...
      if (ownIdAttr.isNillable()) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "EntityType [%s] ID attribute [%s] cannot be nillable",
                    entityType.getId(), ownIdAttr.getName())));
      }
    } else {
  ...
  static void validateOwnAttributes(EntityType entityType) {
    ...
    if (asStream(entityType.getAllAttributes()).collect(toList()).isEmpty()) {
      throw new MolgenisValidationException(
          new ConstraintViolation(
              format(
                  "EntityType [%s] does not contain any attributes. Did you use the correct package+entity name combination in both the entities as well as the attributes sheet?",
                  entityType.getId())));
    }
  ...
  /**
   * Validates the entity ID: - Validates that the entity ID does not contain illegal characters and
