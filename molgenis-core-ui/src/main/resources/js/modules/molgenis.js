...
function createInput(attr, attrs, val, lbl) {
	...
	case 'DATE_TIME':
		...
		var $input = createBasicInput('text', attrs, val).addClass('form-control').attr('data-date-format', dataType === 'DATE' ? 'YYYY-MM-DD' : 'YYYY-MM-DDTHH:mm:ssZZ')
				.appendTo($div);
		if (attr.nillable) {
		...
		return $div;
	case 'DECIMAL':
		var input = createBasicInput('number', $.extend({}, attrs, {
			'step' : 'any'
	...
	var createAttrsValue = function(attrs) {
		...
		for ( var key in attrs) {
			if (attrs.hasOwnProperty(key)) {
				if (attrs[key]) {
					if (attrs[key] === '*') {
						items.push(encodeURIComponent(key) + '(*)'); // do
						// not
						// encode
						// wildcard
						// and
						...
						items.push(encodeURIComponent(key) + '(' + createAttrsValue(attrs[key]) + ')'); // do
						// not
						// encode
						// parenthesis
					}
...
(function($) {
	// Creating an internal undef value is safer than using undefined, in case
	// it
	...
	var deparam = $.deparam = function(text, reviver) {
		...
		$.each(text.replace(/\+/g, ' ').split('&'), function(index, pair) {
			...
			if ($.isFunction(reviver)) {
				...
				// If true was passed, use the built-in $.deparam.reviver
				// function.
				value = deparam.reviver(key, value);
			}

			if (last) {
				// Complex key, like 'a[]' or 'a[b][c]'. At this point, the keys
				// array
				// might look like ['a', ''] (array) or ['a', 'b', 'c']
				// (object).
				for (; i <= last; i++) {
					// If the current key part was specified, use that value as
					// the array
					// index or object key. If omitted, assume an array and use
					// the
					// array's length (effectively an array push).
					key = keys[i] !== '' ? keys[i] : current.length;
					if (i < last) {
						// If not the last key part, update the reference to the
						// current
						// object/array, creating it if it doesn't already exist
						// AND there's
						// a next key. If the next key is non-numeric and not
						// empty string,
				...
				if ($.isArray(result[key])) {
					// If the key already exists, and is an array, push the new
					// value onto
					// the array.
					result[key].push(value);
				} else if (key in result) {
					// If the key already exists, and is NOT an array, turn it
					// into an
