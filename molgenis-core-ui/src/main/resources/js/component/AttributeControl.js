 ...
 * 
 * @memberOf AttributeControl
 */
var AttributeControl = React.createClass({
	mixins : [ DeepPureRenderMixin, I18nStringsMixin, AttributeLoaderMixin ],
	displayName : 'AttributeControl',
	propTypes : {
		attr : React.PropTypes.oneOfType([ React.PropTypes.string, React.PropTypes.object ]),
		// optional overwrite for attr.required
		required : React.PropTypes.bool,
		// optional overwrite for attr.readOnly
		readOnly : React.PropTypes.bool,
		// optional overwrite for attr.visible
		visible : React.PropTypes.bool,
		// optional overwrite for attr.disabled
	...
	render : function() {
		...
		if (attr.expression === undefined) {
			...
			case 'CATEGORICAL':
				...
				if (CategoricalControl === RadioGroup && controlProps.required === false) {
					options = options.concat({
						value : '',
						label : this.state.i18nStrings.form_bool_missing
					});
				}
			...
			case 'XREF':
				return this._createEntitySelectBox(controlProps, props.multiple || false, props.placeholder || this.state.i18nStrings.form_xref_control_placeholder,
						props.value);
			case 'MREF':
				return this._createEntitySelectBox(controlProps, props.multiple || true, props.placeholder || this.state.i18nStrings.form_mref_control_placeholder,
						props.value);
			case 'SCRIPT':
				return CodeEditor(_.extend({}, controlProps, {
					placeholder : this.props.placeholder
	...
	_handleValueChange : function(event) {
		this.props.onValueChange(_.extend({}, event, {
	...
	_createFileControl : function(controlProps) {
		return Input(_.extend({}, controlProps, {
			...
			onValueChange : function(event) {
				event.value = _.extend({}, this.state.attr.refEntity, {
	...
	_createEntitySelectBox : function(controlProps, multiple, placeholder, value) {
		return EntitySelectBox(_.extend({}, controlProps, {
			...
			// TODO same for CATEGORICAL_MREF
			value : multiple ? this._pagedValueToValue(value) : value,
	...
	_onAttrInit : function() {
		...
		if (attr.fieldType === 'CATEGORICAL' || attr.fieldType === 'CATEGORICAL_MREF') {
			...
			// FIXME probles in case of large number of categories
