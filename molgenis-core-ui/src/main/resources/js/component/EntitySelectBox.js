	...
	var EntitySelectBox = React.createClass({
		...
		render : function() {
			...
			var options = {
				...
				placeholder : props.placeholder || ' ', // cannot be an empty
														// string
		...
		_select2FormatResult : function(item) {
			...
			if (attrs.length > 1) {
				...
				// FIXME fix in case of 5, 7 etc. lookup attributes
				var width = Math.round(12 / attrs.length);
				for (var i = 0; i < attrs.length; ++i) {
		...
		_getAttrs : function() {
			// display lookup attributes in dropdown or label attribute if no
			// lookup attributes are defined
			var entity = this.state.entity;
