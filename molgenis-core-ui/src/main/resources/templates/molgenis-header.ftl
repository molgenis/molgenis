...
<head>
  ...
  <link rel="icon" href="<@resource_href "/img/favicon.ico"/>" type="image/x-icon">
    ...
    <#if !version?? || version == 1>
        ...
        <link rel="stylesheet"
              href="<@theme_href "/css/bootstrap-3/${app_settings.bootstrapTheme?html}"/>"
      ...
      <#if app_settings.logoTopHref?has_content>
        <link rel="stylesheet" href="<@resource_href "/css/molgenis-top-logo.css"/>"
              type="text/css"></#if>

      <#if app_settings.cssHref?has_content>
        <link rel="stylesheet" href="<@resource_href "/css/${app_settings.cssHref?html}"/>"
              type="text/css"></#if>
    ...
    <#-- Include bootstrap 4 theme CSS for Vue plugins -->
        <link rel="stylesheet"
              href="<@theme_href "/css/bootstrap-4/${app_settings.bootstrapTheme?html}"/>"
              type="text/css" id="bootstrap-theme">

    <#-- Add spacing to body to show content below fixed-top navigation bar -->
        <link rel="stylesheet" href="<@resource_href "/css/bootstrap-4/menu/fixed-top-menu.css"/>"
              type="text/css">

    <#-- Make footer sticky -->
        <link rel="stylesheet" href="<@resource_href "/css/bootstrap-4/footer/sticky-footer.css"/>"
              type="text/css">
...
<#-- Load css specified by plugins -->
    <#list css as css_file_name>
        <link rel="stylesheet" href="<@resource_href "/css/${css_file_name?html}"/>"
              type="text/css">
    </#list>
...
<body>
  ...
  <#-- VUE -->
        <script type="text/javascript">
          window.molgenisMenu = {
            menu: ${menu}
                <#if app_settings.logoTopHref??>,
                  topLogo: '${app_settings.logoTopHref}'</#if>
                <#if app_settings.logoTopHref??>,
                  topLogoMaxHeight: ${app_settings.logoTopMaxHeight}</#if>
                <#if app_settings.logoNavBarHref?has_content>,
                  navBarLogo: '${app_settings.logoNavBarHref}'</#if>
                <#if plugin_id??>,
                  selectedPlugin: '${plugin_id}'</#if>
            ,
...
<div class="container-fluid"
  ...
  <#if plugin_show_settings_cog>
        <div class="row">
          <div class="col-md-12">
                <span class="glyphicon glyphicon-cog pull-right plugin-settings-btn"
                      aria-hidden="true"
    ...
    <nav class="navbar navbar-default navbar-fixed-top" style="margin-bottom: 10px"
      ...
      <div class="container-fluid">
            ...
            <header id="top-logo-banner" style="height: ${app_settings.logoTopMaxHeight}px">
              ...
              <a href="/"><img id="logo-top" src="${app_settings.logoTopHref?html}" alt=""
                               border="0"
                ...
                <#list menu.items as item>
                  <#if !item.isMenu() && item.label == "Home" && app_settings.logoNavBarHref?has_content>
                        <a class="navbar-brand" href="/menu/${menu.id?html}/${item.url?html}">
                          <img class="img-responsive molgenis-navbar-logo"
                               style="height:2em;font-size:16px;"
        ...
        <div class="collapse navbar-collapse" id="bs-molgenis-navbar">
          <ul class="nav navbar-nav">
                    ...
                    <#-- Single menu items -->
                        <#if !item.isMenu()>
                          <#if item.label != "Home" || !app_settings.logoNavBarHref?has_content>
                            ...
                            <#else>
                                    <li><a
                                        href="/menu/${menu.id?url('UTF-8')}/${item.url?html}">${item.label?html}</a>
                                    </li>
                            </#if>
                          </#if>
                ...
                <ul class="nav navbar-nav navbar-right">
                  ...
                  <li>
                    <a id="manual" href="https://molgenis.gitbooks.io/molgenis/content/"
                       target="_blank">Help</a>
                  </li>
                  <li>
                    <form id="logout-form" class="navbar-form" method="post" action="/logout">
                      <button id="signout-button" type="button" class="btn btn-primary">Sign out
                      </button>
                    </form>
                  </li>
                </ul>
