        ...
        for (var i = 0; i < rules.length; ++i) {
            ...
            switch (rule.operator) {
                case 'SEARCH':
                    ...
                    rsql += encodeURIComponent(field) + '=q=' + this.toRsqlValue(rule.value);
                    break;
                case 'EQUALS':
                    rsql += encodeURIComponent(rule.field) + '==' + this.toRsqlValue(rule.value);
                    break;
                case 'IN':
                    ...
                    break;
                case 'LESS':
                    rsql += encodeURIComponent(rule.field) + '=lt=' + this.toRsqlValue(rule.value);
                    break;
                case 'LESS_EQUAL':
                    rsql += encodeURIComponent(rule.field) + '=le=' + this.toRsqlValue(rule.value);
                    break;
                case 'GREATER':
                    rsql += encodeURIComponent(rule.field) + '=gt=' + this.toRsqlValue(rule.value);
                    break;
                case 'GREATER_EQUAL':
                    rsql += encodeURIComponent(rule.field) + '=ge=' + this.toRsqlValue(rule.value);
                    break;
                case 'RANGE':
                    rsql += encodeURIComponent(rule.field) + '=rng=' + '(' + this.toRsqlValue(rule.value[0]) + ',' + this.toRsqlValue(rule.value[1]) + ')';
                    break;
                case 'LIKE':
                    rsql += encodeURIComponent(rule.field) + '=like=' + this.toRsqlValue(rule.value);
                    break;
                case 'NOT':
                    rsql += encodeURIComponent(rule.field) + '!=' + this.toRsqlValue(rule.value);
                    break;
                ...
                case 'NESTED':
                    ...
                    if (rule.nestedRules.length > 0) {
                        rsql += this.createRsqlQuery(rule.nestedRules);
                    }
                    if (rule.nestedRules.length > 1) {
                        rsql += ')';
                    }
        ...
        if (options) {
            ...
            if (options.q) {
                if (options.q.length > 0) {
                    items.push('q=' + this.createRsqlQuery(options.q));
                }
            }
            if (options.aggs) {
                items.push('aggs=' + this.createRsqlAggregateQuery(options.aggs));
            }
            if (options.attrs) {
                items.push('attrs=' + this.createAttrsValue(options.attrs));
            }
            if (options.sort) {
                items.push('sort=' + this.createSortValue(options.sort));
            }
...
export default new RestClientV2();
