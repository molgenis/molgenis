...
var Form = React
	.createClass({
		...
		propTypes : {
			...
			// whether or not to render form in a modal dialog
			modal : React.PropTypes.bool,
			// whether or not to show a control to filter optional form
			// fields
			enableOptionalFilter : React.PropTypes.bool,
			// save form control values on blur
			saveOnBlur : React.PropTypes.bool,
			// whether or not to show a form index to navigate to form
			// controls
			enableFormIndex : React.PropTypes.bool,
			// whether or not to show a alert messages above the index
			// to navigate to form controls. (Only works if the
			// enableFormIndex prop is set to true)
			enableAlertMessageInFormIndex : React.PropTypes.bool,
			// whether or not to show not-visible attributes
			showHidden : React.PropTypes.bool,
			// whether to show 'select all' and 'hide all' links under
			// the categorical mref checkboxes
			categoricalMrefShowSelectAll : React.PropTypes.bool,
			// whether to show a '*' after the label when an attribute
			// is not nillable
		...
		_setDefaultValue : function(attr, entityInstance) {
			try {
				...
				case 'CATEGORICAL':
					...
					// TODO: both name and value for the label attribute
					// are missing!
				...
				case 'CATEGORICAL_MREF':
					entityInstance[attr.name] = {
						...
						items : attr.defaultValue.split(',').map(function(idValue) {
							var value = {};
							// TODO: both name and value for the label
					...
					break;
				case 'COMPOUND':
					// makes no sense to have a defaultValue for a
					// compound
					break;
				default:
		...
		_renderForm : function() {
			...
			var formProps = {
				...
				action : action,
				method : method,
				// TODO use multipart/form-data if form contains one or
				// more file inputs
			...
			var formControlsProps = {
				...
				// FIXME replace value with entity instance
				value : this.state.entityInstance,
			...
			var Filter = this.props.enableOptionalFilter ? (div({
				className : 'row',
				style : {
					textAlign : 'right'
				},
			...
			var Form = (JQueryForm(formProps, FormControls(formControlsProps), this.props.mode !== 'view'
					&& !(this.props.mode === 'edit' && this.props.saveOnBlur) ? FormButtons({
				mode : this.props.mode,
				formLayout : this.props.formLayout,
				colOffset : this.props.colOffset,
			...
			if (this.props.enableFormIndex) {
				FormWithMessageAndFilter = (div(null, (this.props.enableAlertMessageInFormIndex ? null : ErrorMessageAlertMessage),
						(this.props.enableAlertMessageInFormIndex ? null : SubmitAlertMessage), Filter, Form));
				return (div({
					className : 'row'
				}, div({
					className : 'col-md-10'
				}, FormWithMessageAndFilter), div({
					className : 'col-md-2'
				}, FormIndex({
					entity : this.state.entity,
		...
		_handleValueChange : function(e) {
			...
			this._validate(attr, value, function(validationResult) {
					...
					_.each(this.state.entity.allAttributes, function(entityAttr) {
						if (entityAttr.visibleExpression) {
							...
							if (entityAttr.visible === false) {
								this._updateErrorMessages(entityAttr, {
									valid : true
								});
							}
						}
				...
				if (validationResult.valid === true && this._doPersistAttributeValue(attr)) {
					// persist changes for controls that do not have a
					// blur event
					switch (attr.fieldType) {
					case 'BOOL':
					case 'CATEGORICAL':
		...
		submit : function(e) {
			...
			getPromises(promises).done(function(results) {
				...
				if (valid) {
					// TODO remove jquery form submit workaround, see
					// also componentDidMount in JQueryForm.js
		...
		_handleSubmitSuccess : function(responseText, statusText, xhr, element) {
			...
			var stateProps = {
				submitMsg : {
					type : 'success',
					message : this.state.entity.label + ' ' + message
				},
				errorMessages : {},
				validate : false,
			};
		...
		_validate : function(attr, value, callback) {
			// apply validation rules, not that IE9 does not support
			...
			if (!computed) {
				if (attr.nillable === false && type !== 'CATEGORICAL_MREF' && type !== 'MREF' && nullOrUndefinedValue && !attr.auto) { // required
					// value
					// constraint
					if (attr.visibleExpression === undefined || this._resolveBoolExpression(attr.visibleExpression, entityInstance) === true) {
						errorMessage = 'Please enter a value.';
					}
				} else if (attr.nillable === false && (type === 'CATEGORICAL_MREF' || type === 'MREF') && (nullOrUndefinedValue || value.items.length === 0)) { // required
					// value
					// constraint
					if (attr.visibleExpression === undefined || this._resolveBoolExpression(attr.visibleExpression, entityInstance) === true) {
						errorMessage = 'Please enter a value.';
				...
				// value uniqueness constraint
					...
					api.getAsync(this.state.entity.hrefCollection, {
						q : {
							q : rules
						}
					}, function(data) {
		...
		_getValue : function(entityInstance, attr) {
			// Please don't manipulate the values here. It is not the
			// place to do it!
