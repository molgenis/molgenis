...
var Input = React.createClass({
	...
	getInitialState : function() {
		return this._isRadioOrCheckbox() ? {
	...
	componentWillReceiveProps : function(nextProps) {
		this.setState(this._isRadioOrCheckbox() ? {
	...
	render : function() {
		...
		if (props.readOnly && this._isRadioOrCheckbox()) {
			// readonly attribute doesn't work on checkboxes and radio
			// buttons:
			...
			if (this.state.checked) {
				// for checked checkboxes/radio buttons submit the value of
				// a hidden input,
				// disable visible input to display a readonly view to the
				// user
				return div({}, input({
					type : 'hidden',
					name : props.name,
				...
				// values of unchecked checkboxes/radio buttons are never
				// submitted:
			...
			if (this._isFile()) {
				...
				}, 'Browse...', input(_.extend(inputProps, {
	...
	_handleChange : function(event) {
		...
		if (this._isRadioOrCheckbox()) {
			...
			// apply constraint: maximum number of characters allowed in
			// input
			if (this.props.maxLength) {
				value = value.substr(0, this.props.maxLength);
			}

			// number change event with invalid number value (e.g. '1abc')
			// will trigger a change event with value '',
			// use input validation status to determine is the value was
			// cleared or if we are dealing with an invalid value.
			// Note: this does not apply to IE9 which does not support the
			// input number type and the validation API.
	...
	_handleChangeOrBlur : function(value, checked, callback) {
		...
		if (this._isRadioOrCheckbox()) {
			valueEvent = {
				value : this._emptyValueToNull(value),
				checked : checked
			};
		} else {
	...
	_handleFileBrowseClick : function() {
		var input = $(this.refs.input.getDOMNode()), value = input.val(), label = this._toFileLabel(value);
