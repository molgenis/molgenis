/**
 * @module EntityInstanceLoaderMixin
 */
"use strict";
...
/**
 * Loads an entity instance (row of data) from this.props.entityInstance and
 * stores in this.state.entityInstance
 * 
...
var EntityInstanceLoaderMixin = {
	...
	_loadEntityInstance : function(entity, href) {
		if (entity && entity.name) {
			...
			api.getAsync(href, expands.length > 0 ? {
				'expand' : expands
			} : undefined).done(function(entityInstance) {
				if (this.isMounted()) {
					this._setEntityInstance(entity, entityInstance);
				}
			}.bind(this));
