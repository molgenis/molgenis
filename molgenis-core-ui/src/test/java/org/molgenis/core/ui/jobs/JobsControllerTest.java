...
public class JobsControllerTest extends AbstractMockitoTest
{
	...
	@Mock
	private EntityType jobExecutionEntityType;
	@Mock
	...
	public void beforeClass()
	{
		jobsController = new JobsController(userAccountService, dataService, jobMetaDataMetaData, jobScheduler,
				menuReaderService, userPermissionEvaluator);
	}
	...
	{
		EntityType entityType = mock(EntityType.class);
		EntityType extendsEntityType = when(mock(EntityType.class).getId()).thenReturn("not_sys_job_JobExecution")
																		   .getMock();
	...
	public void testIsAllowedJobExecutionEntityTypeNoReadDataPermissions()
	{
		...
		EntityType entityType = when(mock(EntityType.class).getId()).thenReturn("MyJobExecution").getMock();
		EntityType extendsEntityType = when(mock(EntityType.class).getId()).thenReturn("sys_job_JobExecution")
																		   .getMock();
		when(entityType.getExtends()).thenReturn(extendsEntityType);
		assertFalse(jobsController.isAllowedJobExecutionEntityType(entityType));
	}
	...
	public void testIsAllowedJobExecutionEntityTypeReadDataPermissions()
	{
		...
		EntityType extendsEntityType = when(mock(EntityType.class).getId()).thenReturn("sys_job_JobExecution")
																		   .getMock();
		when(entityType.getExtends()).thenReturn(extendsEntityType);
		when(userPermissionEvaluator.hasPermission(new EntityTypeIdentity(entityType), READ_DATA)).thenReturn(true);
