...
public class ImportWizardControllerMockMVCTest extends AbstractMockitoTest {
  ...
  @Mock private org.molgenis.data.meta.MetaDataService metaDataService;
  ...
  public void testInvalidAction() throws Exception {
    ...
    mockMvc
        .perform(fileUpload(new URI(URI + "/importFile")).file(vcfFile).param("action", "bug"))
  ...
  public void testRepositoryExists() throws Exception {
    ...
    mockMvc
        .perform(
            fileUpload(new URI(URI + "/importFile"))
                .file(vcfFile)
                .param("action", "add")
                .param("entityTypeId", "test"))
        .andExpect(status().isBadRequest())
