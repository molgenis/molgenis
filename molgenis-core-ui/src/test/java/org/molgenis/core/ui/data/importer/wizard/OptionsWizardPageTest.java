...
public class OptionsWizardPageTest extends AbstractMockitoTest
{
	...
	{
		optionsWizardPage = new OptionsWizardPage(fileRepositoryCollectionFactory, importServiceFactory, dataService,
	...
	public void testHandleRequestEntitiesInPackages()
	{
		...
		when(fileRepositoryCollectionFactory.createFileRepositoryCollection(file)).thenReturn(fileRepositoryCollection);
		...
		when(importService.validateImport(file, fileRepositoryCollection)).thenReturn(entitiesValidationReport);
		when(importServiceFactory.getImportService(file, fileRepositoryCollection)).thenReturn(importService);
		...
		assertEquals(optionsWizardPage.handleRequest(httpServletRequest, bindingResult, wizard),
	...
	{
		HttpServletRequest httpServletRequest = mock(HttpServletRequest.class);
		when(httpServletRequest.getParameter("entity_option")).thenReturn("MyAction");
		BindingResult bindingResult = mock(BindingResult.class);
		ImportWizard wizard = mock(ImportWizard.class);
		File file = mock(File.class);
		...
		ImportService importService = mock(ImportService.class);
		EntitiesValidationReport entitiesValidationReport = mock(EntitiesValidationReport.class);
		when(entitiesValidationReport.getSheetsImportable()).thenReturn(singletonMap("MyEntityType", true));
		when(importService.validateImport(file, fileRepositoryCollection)).thenReturn(entitiesValidationReport);
		when(importServiceFactory.getImportService(file, fileRepositoryCollection)).thenReturn(importService);
		MetaDataService metaDataService = mock(MetaDataService.class);
