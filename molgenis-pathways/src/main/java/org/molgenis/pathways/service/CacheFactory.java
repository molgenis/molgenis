...
public class CacheFactory
{
	 ...
	 * 
	 * @param loader
	 *            {@link RemoteFunction} that loads a value for the cache
	 ...
	 * 
	 * @param loader
	 *            {@link RemoteFunction} that loads the pathways
	 * @param filter
	 *            {@link BiPredicate} to filter the loaded pathways
	 * @param pathwayTransformer
	 *            {@link Function} that transforms the loaded pathways to {@link Pathway}s
			...
			Function<Result, Pathway> pathwayTransformer)
	{
		return loadingCache(params -> Arrays.stream(loader.apply(params))
				.filter((result) -> filter.test(params, result)).map(pathwayTransformer)
