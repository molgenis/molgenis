...
public class UserAuthorityMetaData extends DefaultEntityMetaData {
    ...
    public UserAuthorityMetaData() {
        super(ENTITY_NAME);
        addAttribute(UserAuthority.ID).setAuto(true).setVisible(false)
                .setDescription("").setIdAttribute(true).setNillable(false).setLabelAttribute(true);
        addAttribute(UserAuthority.MOLGENISUSER).setDataType(XREF).setRefEntity(new MolgenisUserMetaData()).setAggregateable(true).setDescription("").setNillable(false);
