...
public class MetaDataSearchServiceImpl implements MetaDataSearchService
{
	...
	public List<PackageSearchResultItem> findRootPackages(String searchTerm)
	{
		...
		else
		{
			...
			{
				/*
				 * TODO: implement Entity entityMetaData =
				 * attributeMetaData.getEntity(AttributeMetaDataMetaData.ENTITY); if (entityMetaData != null) { Package
				 * p = getRootPackage(entityMetaData); if (p != null) { String matchDesc = "Matched: attribute '" +
				 * attributeMetaData.getString(AttributeMetaDataMetaData.NAME) + "' of entity '" +
				 * entityMetaData.getString(EntityMetaDataMetaData.SIMPLE_NAME) + "'"; PackageSearchResultItem item =
				 * new PackageSearchResultItem(p.getRootPackage(), matchDesc); if ((p != null) &&
				 * !results.contains(item)) results.add(item); } }
				 */
			}
		}
