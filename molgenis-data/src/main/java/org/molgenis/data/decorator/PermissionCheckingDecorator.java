...
public class PermissionCheckingDecorator<E extends Entity> extends AbstractRepositoryDecorator<E>
{
	...
	Iterator<E> iterator()
	{
		return Streams.stream(delegate().iterator()).filter(permissionChecker::isReadAllowed).iterator();
	}
	...
	public void forEachBatched(Fetch fetch, Consumer<List<E>> consumer, int batchSize)
	{
		delegate().forEachBatched(fetch, entities -> consumer.accept(
				entities.stream().filter(permissionChecker::isReadAllowed).collect(toList())), batchSize);
	}
	...
	public long count(Query<E> q)
	{
		...
		Stream<E> permittedEntityStream = delegate().findAll(qWithoutLimitOffset)
													.filter(permissionChecker::isCountAllowed);
	...
	public void deleteAll(Stream<Object> ids)
	{
		...
		partition(ids.iterator(), BATCH_SIZE).forEachRemaining(batchIds ->
		{
			List<E> entities = delegate().findAll(batchIds.stream()).collect(toList());
			delegate().deleteAll(entities.stream().filter(permissionChecker::isDeleteAllowed).map(Entity::getIdValue));
		});
	}
