...
public interface Repository<E extends Entity> extends Iterable<E>, Closeable {
  ...
  default void upsertBatch(List<E> entities) {
    Set<Object> existingIDs =
        getExistingIDs(entities.stream().map(Entity::getIdValue).collect(toSet()));
    Map<Boolean, List<E>> partitioned =
        entities
            .stream()
   ...
   *
   * <p>Returns null if not exists.
   *
   * <p>Returns first result if multiple found
   *
   * @param q query
   */
  E findOne(Query<E> q);
