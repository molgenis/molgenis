...
public class PermissionCheckingDecoratorTest extends AbstractMockitoTest
{
	...
	@Mock
	private PermissionChecker<Entity> permissionChecker;
	...
	{
		permissionCheckingDecorator = new PermissionCheckingDecorator<>(delegateRepository, permissionChecker);
	}
	...
	public void testIterator()
	{
		...
		when(delegateRepository.iterator()).thenReturn(asList(permittedEntity, notPermittedEntity).iterator());
		when(permissionChecker.isReadAllowed(permittedEntity)).thenReturn(true);
		assertEquals(newArrayList(permissionCheckingDecorator.iterator()), singletonList(permittedEntity));
	}
	...
	public void testForEachBatched()
	{
		...
		doAnswer(invocation ->
		{
			((Consumer<List<Entity>>) invocation.getArgument(1)).accept(asList(permittedEntity, notPermittedEntity));
			return null;
	...
	public void testCount()
	{
		Entity permittedEntity = mock(Entity.class);
		Entity notPermittedEntity = mock(Entity.class);
		when(delegateRepository.findAll(new QueryImpl<>().offset(0).pageSize(Integer.MAX_VALUE))).thenReturn(
				Stream.of(permittedEntity, notPermittedEntity));
	...
	public void testFindAllQuery()
	{
		...
		when(delegateRepository.findAll(new QueryImpl<>().offset(0).pageSize(Integer.MAX_VALUE))).thenReturn(
				Stream.of(permittedEntity, notPermittedEntity));
		when(permissionChecker.isReadAllowed(permittedEntity)).thenReturn(true);
		assertEquals(permissionCheckingDecorator.findAll(new QueryImpl<>()).collect(toList()),
	...
	public void testFindAllQuerySkipLimit()
	{
		...
		when(delegateRepository.findAll(new QueryImpl<>().offset(0).pageSize(Integer.MAX_VALUE))).thenReturn(
				Stream.of(permittedEntity0, notPermittedEntity0, permittedEntity1, notPermittedEntity1,
						permittedEntity2));
		...
		assertEquals(permissionCheckingDecorator.findAll(new QueryImpl<>().offset(1).pageSize(1)).collect(toList()),
	...
	public void testFindOneQuery()
	{
		...
		Entity permittedEntity = mock(Entity.class);
		when(delegateRepository.findAll(new QueryImpl<>().offset(0).pageSize(Integer.MAX_VALUE))).thenReturn(
				Stream.of(notPermittedEntity, permittedEntity));
	...
	public void testFindAllStream()
	{
		...
		when(permissionChecker.isReadAllowed(permittedEntity)).thenReturn(true);
		assertEquals(permissionCheckingDecorator.findAll(Stream.of("permittedId", "notPermittedId")).collect(toList()),
				singletonList(permittedEntity));
	}
	...
	public void testFindAllStreamFetch()
	{
		Entity permittedEntity = mock(Entity.class);
		Fetch fetch = mock(Fetch.class);
		when(delegateRepository.findAll(any(Stream.class), eq(fetch))).thenAnswer(invocation ->
		{
			((Stream<Object>) invocation.getArguments()[0]).count(); // consume stream
			return Stream.of(permittedEntity);
		});
		when(permissionChecker.isReadAllowed(permittedEntity)).thenReturn(true);
		assertEquals(permissionCheckingDecorator.findAll(Stream.of("permittedId", "notPermittedId"), fetch)
												.collect(toList()), singletonList(permittedEntity));
	}
	...
	{
		Entity permittedEntity = when(mock(Entity.class).getIdValue()).thenReturn("permittedId").getMock();
		Entity notPermittedEntity = mock(Entity.class);
		when(permissionChecker.isDeleteAllowed(permittedEntity)).thenReturn(true);
		when(delegateRepository.findAll(any(Stream.class))).thenReturn(Stream.of(permittedEntity, notPermittedEntity));
	...
	public void testAggregateSystemUser()
	{
		...
		try
		{
			...
			securityContext.setAuthentication(new UsernamePasswordAuthenticationToken("principal", "credentials",
					singleton(new SimpleGrantedAuthority("ROLE_SYSTEM"))));
