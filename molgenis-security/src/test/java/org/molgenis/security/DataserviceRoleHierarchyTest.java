...
public class DataserviceRoleHierarchyTest extends AbstractMockitoTest
{
	...
	public void setUpBeforeMethod()
	{
		...
		when(dataService.findAll(eq(ROLE), queryCaptor.capture(), eq(Role.class))).thenReturn(
				Stream.of(viewer, bbmriViewer, su, aclTakeOwnership, aclModifyAuditing, aclGeneralChanges,
						dataExplorer));
	}
	...
	public void testGetReachableGrantedAuthoritiesSu()
	{
		...
		when(su.getIncludes()).thenReturn(ImmutableList.of(aclTakeOwnership, aclModifyAuditing, aclGeneralChanges));
		...
		assertEquals((Set) molgenisRoleHierarchy.getReachableGrantedAuthorities(
				singletonList(new SimpleGrantedAuthority("ROLE_SU"))),
				ImmutableSet.of(new SimpleGrantedAuthority("ROLE_ACL_TAKE_OWNERSHIP"),
	...
	public void testGetReachableGrantedAuthoritiesDeep()
	{
		...
		assertEquals((Set) molgenisRoleHierarchy.getReachableGrantedAuthorities(
				...
				ImmutableSet.of(new SimpleGrantedAuthority("ROLE_BBMRI_VIEWER"),
						new SimpleGrantedAuthority("ROLE_VIEWER"), new SimpleGrantedAuthority("ROLE_DATA_EXPLORER")));
	}
}