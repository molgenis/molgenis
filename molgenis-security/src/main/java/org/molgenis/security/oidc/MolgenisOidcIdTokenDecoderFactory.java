...
public class MolgenisOidcIdTokenDecoderFactory implements JwtDecoderFactory<ClientRegistration> {
  ...
  private NimbusJwtDecoder buildDecoder(ClientRegistration clientRegistration) {
    ...
    if (jwsAlgorithm != null
        && SignatureAlgorithm.class.isAssignableFrom(jwsAlgorithm.getClass())) {
  ...
  public void setJwtValidatorFactory(
      Function<ClientRegistration, OAuth2TokenValidator<Jwt>> jwtValidatorFactory) {
    Assert.notNull(jwtValidatorFactory, "jwtValidatorFactory cannot be null");
    this.jwtValidatorFactory = jwtValidatorFactory;
  }

  /**
   * Sets the resolver that provides the expected {@link JwsAlgorithm JWS algorithm} used for the
   * signature or MAC on the {@link OidcIdToken ID Token}. The default resolves to {@link
   * SignatureAlgorithm#RS256 RS256} for all {@link ClientRegistration clients}.
   *
   * @param jwsAlgorithmResolver the resolver that provides the expected {@link JwsAlgorithm JWS
   *     algorithm} for a specific {@link ClientRegistration client}
   */
  public void setJwsAlgorithmResolver(
      Function<ClientRegistration, JwsAlgorithm> jwsAlgorithmResolver) {
   ...
   * an {@link OidcIdToken}. The default is {@link ClaimTypeConverter} for all {@link
