...
public class PermissionSystemServiceImpl implements PermissionSystemService
{
	...
	@Autowired
	public PermissionSystemServiceImpl(DataService dataService, UserAuthorityFactory userAuthorityFactory)
	{
	...
	public void giveUserEntityPermissions(SecurityContext securityContext, List<String> entities)
	{
				...
				.contains(new SimpleGrantedAuthority("ROLE_SYSTEM")))
		{
			User user = dataService
					.findOne(USER, new QueryImpl<User>().eq(UserMetaData.USERNAME, SecurityUtils.getUsername(auth)),
							User.class);
