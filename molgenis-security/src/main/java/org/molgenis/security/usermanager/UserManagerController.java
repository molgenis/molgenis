...
public class UserManagerController extends MolgenisPluginController {
	public final static String URI = MolgenisPluginController.PLUGIN_URI_PREFIX
			+ "usermanager";
	...
	public UserManagerController(UserManagerService pluginUserManagerService) {
		...
		if (pluginUserManagerService == null) {
			throw new IllegalArgumentException(
					"PluginUserManagerService is null");
		}
	...
	public String init(Model model) {
		model.addAttribute("users",
				this.pluginUserManagerService.getAllMolgenisUsers());
		model.addAttribute("groups",
				this.pluginUserManagerService.getAllMolgenisGroups());
	...
	@ResponseStatus(HttpStatus.OK)
	public void setActivation(@PathVariable String type,
			@PathVariable Integer id, @PathVariable Boolean active) {
		if ("user".equals(type)) {
	...
	@ResponseStatus(HttpStatus.OK)
	public void changeGroupMembership(@PathVariable Integer userId,
			@PathVariable Integer groupId, @PathVariable Boolean member) {
		if (member)
			this.pluginUserManagerService.addUserToGroup(groupId, userId);
		if (!member)
			this.pluginUserManagerService.removeUserFromGroup(groupId, userId);
	}
}
