...
<div class="row">
	<div class="col-md-12">
		<form class="form-horizontal" id="entity-form" data-id="${entityName?html}" role="form" method="POST" action="/api/v1/${entityName?html}<#if form.primaryKey?is_number>/${form.primaryKey?c?html}<#else>/${form.primaryKey?html}</#if>?_method=PUT">
