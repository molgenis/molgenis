...
import static org.molgenis.data.meta.MetaIdGenerator.generateId;
...
class PostgreSqlNameGenerator
{
	 ...
	 */
	static String getTableName(EntityType entityType, boolean quotedIdentifier)
	{
		return getTableName(entityType, quotedIdentifier, MAX_IDENTIFIER_LENGTH);
	}

	private static String getTableName(EntityType entityType, boolean quotedIdentifier, int maxLength)
	{
		String identifier = generateId(entityType, maxLength);
	...
	static String getJunctionTableName(EntityType entityType, Attribute attr)
	{
		...
		String entityPart = generateId(entityType, (MAX_IDENTIFIER_LENGTH - nrAdditionalChars) / 2);
		String attrPart = generateId(attr, (MAX_IDENTIFIER_LENGTH - nrAdditionalChars) / 2);
	...
	static String getJunctionTableIndexName(EntityType entityType, Attribute attr, Attribute idxAttr)
	{
		...
		String entityPart = generateId(entityType, (MAX_IDENTIFIER_LENGTH - nrAdditionalChars) / 3);
		String attrPart = generateId(attr, (MAX_IDENTIFIER_LENGTH - nrAdditionalChars) / 3);
		String idxAttrPart = generateId(idxAttr, (MAX_IDENTIFIER_LENGTH - nrAdditionalChars) / 3);
	...
	static String getColumnName(Attribute attr, boolean quotedIdentifier)
	{
		String identifier = generateId(attr, MAX_IDENTIFIER_LENGTH);
		return quotedIdentifier ? getQuotedIdentifier(identifier) : identifier;
	}
	...
	static String getFilterColumnName(Attribute attr, int filterIndex)
	{
		...
		String attrId = generateId(attr, MAX_IDENTIFIER_LENGTH - nrAdditionalChars);
	...
	static String getFunctionValidateUpdateName(EntityType entityType)
	{
		...
		int maxLength = MAX_IDENTIFIER_LENGTH - prefix.length() - 1;
