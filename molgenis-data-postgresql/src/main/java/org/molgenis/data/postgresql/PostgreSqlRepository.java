...
class PostgreSqlRepository extends AbstractRepository {
  ...
  private Multimap<Object, Object> selectMrefIDsForAttribute(
    ...
    jdbcTemplate.query(
        junctionTableSelect,
        getJunctionTableRowCallbackHandler(idAttributeDataType, refIdDataType, mrefIDs),
        ids.toArray());
  ...
  RowCallbackHandler getJunctionTableRowCallbackHandler(
      AttributeType idAttributeDataType,
      AttributeType refIdDataType,
      Multimap<Object, Object> mrefIDs) {
    return row -> {
  ...
  private static Map<String, List<Map<String, Object>>> createMrefMap(
    ...
    for (Entity entity : entitiesBatch) {
      for (Attribute attr : junctionTableAttrs) {
        ...
        // Not-Null constraint doesn't exist for MREF attributes since they are stored in junction
        // tables,
        ...
        for (Entity val : refEntities) {
          Map<String, Object> mref =
              createJunctionTableRowData(seqNr.getAndIncrement(), idAttr, val, attr, entity);
  ...
  void addMrefs(final List<Map<String, Object>> mrefs, final Attribute attr) {
    // database doesn't validate NOT NULL constraint for attribute values referencing multiple
    // entities,
      ...
      if (mve.getMessage().equals(VALUE_TOO_LONG_MSG)) {
        mve =
            new MolgenisValidationException(
                new ConstraintViolation(
                    format(
                        "One of the mref values in entity type [%s] attribute [%s] is too long.",
