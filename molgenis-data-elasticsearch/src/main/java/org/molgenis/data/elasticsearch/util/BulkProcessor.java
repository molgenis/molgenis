...
/**
 * Copied from https://github.com/elasticsearch/elasticsearch/pull/6586 Should be available in Elasticsearch 1.4.0,
 * remove this class when we update to Elasticsearch 1.4.0
 * 
 * 
...
public class BulkProcessor implements Closeable
{
	...
	public static class Builder
	{
		...
		/**
		 * Creates a builder of bulk processor with the client to use and the listener that will be used to be notified
		 * on the completion of bulk requests.
		 */
		...
		/**
		 * Sets the number of concurrent requests allowed to be executed. A value of 0 means that only a single request
		 * will be allowed to be executed. A value of 1 means 1 concurrent request is allowed to be executed while
		 * accumulating new bulk requests. Defaults to <tt>1</tt>.
		 */
		...
		/**
		 * Sets when to flush a new bulk request based on the size of actions currently added. Defaults to <tt>5mb</tt>.
		 * Can be set to <tt>-1</tt> to disable it.
		 */
	...
	BulkProcessor(Client client, Listener listener, @Nullable String name, int concurrentRequests, int bulkActions,
			ByteSizeValue bulkSize, @Nullable TimeValue flushInterval)
	{
		this.client = client;
		this.listener = listener;
		this.name = name;
		...
		if (flushInterval != null)
		{
			this.scheduler = (ScheduledThreadPoolExecutor) Executors.newScheduledThreadPool(
					1,
					EsExecutors.daemonThreadFactory(client.settings(), (name != null ? "[" + name + "]" : "")
							+ "bulk_processor"));
	 ...
	 * Closes the processor. If flushing by time is enabled, then it's shutdown. Any remaining bulk actions are flushed.
	 *
	 * If concurrent requests are not enabled, returns {@code true} immediately. If concurrent requests are enabled,
	 * waits for up to the specified timeout for all bulk requests to complete then returns {@code true}, If the
	 * specified waiting time elapses before all bulk requests complete, {@code false} is returned.
	 *
	 * @param timeout
	...
	public BulkProcessor add(BytesReference data, boolean contentUnsafe, @Nullable String defaultIndex,
			@Nullable String defaultType) throws Exception
	{
		return add(data, contentUnsafe, defaultIndex, defaultType, null);
	}

	public synchronized BulkProcessor add(BytesReference data, boolean contentUnsafe, @Nullable String defaultIndex,
