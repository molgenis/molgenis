...
public class EntityReportPermissionChecker implements PermissionChecker<FreemarkerTemplate>
{
	 ...
	 */
	private boolean hasEntityTypeReportPermission(FreemarkerTemplate freemarkerTemplate,
			EntityTypeReportPermission permission)
	{
		return getEntityTypeId(freemarkerTemplate.getName()).map(EntityTypeIdentity::new)
															.map(id -> permissionEvaluator.hasPermission(id,
																	permission))
															.orElse(true);
	}
