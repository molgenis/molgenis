...
;(function ($, molgenis) {
  ...
  function createDataTable() {
    ...
    $.get('/permission/sys_FreemarkerTemplate/read').done(function (canRead) {
      var tableProperties = {
        ...
        onRowClick:
          doShowGenomeBrowser() && isGenomeBrowserAttributesSelected()
            ? onRowClick
            : null,
      ...
      if (useDataEditRowPlugin) {
        ;(tableProperties.onEditClick = function (tableId, tableRowId) {
          ...
          (tableProperties.onAddClick = function (tableId) {
            window.location.assign(
              window.location.origin + '/plugin/data-row-edit/' + tableId
            )
          })
      }
      Table = React.render(
        molgenis.ui.Table(tableProperties),
  ...
  function doShowGenomeBrowser() {
    ...
    return (
      molgenis.ie9 !== true &&
      molgenis.ie10 !== true &&
      genomebrowserStartAttribute !== undefined &&
      genomebrowserChromosomeAttribute !== undefined &&
      molgenis.dataexplorer.settings['data_genome_browser'] !== false
    )
  }
  ...
  function isGenomeBrowserAttributesSelected() {
    ...
    return (
      attributes.indexOf(genomebrowserChromosomeAttribute) != -1 &&
      attributes.indexOf(genomebrowserStartAttribute) != -1
    )
  }
  ...
  function createGenomeBrowser(specificSettings) {
    ...
    if (showHighlight === true) {
      genomeBrowser.highlightRegion(
        genomeBrowser.chr,
        genomeBrowser.viewStart + 9990,
        genomeBrowser.viewEnd - 9990
      )
    }
  ...
  function setDallianceFilter() {
    $.each(getAttributes(), function (key, attribute) {
      if (attribute === genomebrowserStartAttribute) {
        createFilter(
          attribute,
          Math.floor(genomeBrowser.viewStart).toString(),
          Math.floor(genomeBrowser.viewEnd).toString()
        )
  ...
  function createFilter(attribute, fromValue, toValue, values) {
    var attributeFilter = new molgenis.dataexplorer.filter.SimpleFilter(
      attribute,
      fromValue,
      toValue,
      values
    )
    var complexFilter = new molgenis.dataexplorer.filter.ComplexFilter(
      attribute
    )
    var complexFilterElement =
      new molgenis.dataexplorer.filter.ComplexFilterElement(attribute)
  ...
  $(function () {
    ...
    $(document).on('changeAttributeSelection.data', function (e, data) {
      if (
        Table &&
        Table.isMounted() &&
        molgenis.dataexplorer.getSelectedModule() == 'data'
      ) {
        ...
        for (var attr in treeAttrs) {
          if (tableAttrs[attr] !== undefined && tableAttrs[attr] !== null) {
            ...
            if (
              tableAttrs[attr].hasOwnProperty('*') &&
              treeAttrs.hasOwnProperty(attr)
            ) {
      ...
      if (molgenis.dataexplorer.getSelectedModule() == 'data') {
        if (molgenis.dataexplorer.settings['genomebrowser'] !== 'false') {
          ...
          $.each(data.filters, function () {
            if (
              this.getComplexFilterElements &&
              this.getComplexFilterElements()[0]
            ) {
              if (this.attribute === genomebrowserStartAttribute) {
                setLocation(
                  genomeBrowser.chr,
                  parseInt(
                    this.getComplexFilterElements()[0].simpleFilter.fromValue
                  ),
                  parseInt(
                    this.getComplexFilterElements()[0].simpleFilter.toValue
                  )
                )
              } else if (this.attribute === genomebrowserChromosomeAttribute) {
                setLocation(
                  this.getComplexFilterElements()[0].simpleFilter.getValues()[0],
