...
public class RestControllerIT
{
	...
	public void beforeClass()
	{
		...
		grantSystemRights(adminToken, PATH, SCRIPT_TYPE_PERMISSION_ID, testUserId, "sys_scr_ScriptType", Permission.READ);
		grantSystemRights(adminToken, PATH, SYS_SEC_USER_AUTHORITY_ID, testUserId, "sys_sec_UserAuthority", Permission.COUNT);
	...
	public void getWithoutTokenNotAllowed()
	{
		...
		response.statusCode(UNAUTHORIZED).body("errors.message[0]", equalTo("No [COUNT] permission on entity [sys_FreemarkerTemplate]"));

		response = getWithoutToken("sys_scr_ScriptType");
		response.statusCode(UNAUTHORIZED).body("errors.message[0]", equalTo("No [COUNT] permission on entity [sys_scr_ScriptType]"));

		response = getWithoutToken("sys_sec_UserAuthority");
		response.statusCode(UNAUTHORIZED).body("errors.message[0]", equalTo("No [COUNT] permission on entity [sys_sec_UserAuthority]"));

		response = getWithoutToken("sys_sec_GroupAuthority");
		response.statusCode(UNAUTHORIZED).body("errors.message[0]", equalTo("No [COUNT] permission on entity [sys_sec_GroupAuthority]"));
	}
	...
	public void getWithoutReadPermissionIsNotAllowed()
	{
		...
		response.statusCode(UNAUTHORIZED).body("errors.message[0]", equalTo("No [READ] permission on entity [sys_sec_UserAuthority]"));
	}
	...
	private ValidatableResponse getWithToken(String requestedEntity, String token)
	{
		return given().log().all()
				...
				.contentType(APPLICATION_JSON).when()
