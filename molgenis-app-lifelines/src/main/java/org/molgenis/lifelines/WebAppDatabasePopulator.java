...
{	
	...
	{
		if(appProfile==null||dataManagerPassword==null||researchPassword==null){
			StringBuilder message = new StringBuilder("please configure: ");
			if(appProfile == null) message.append("app.profile(possible values: workspace or website), ");
			if(dataManagerPassword == null) message.append("default datamanager.password, ");
			if(researchPassword == null) message.append("default researcher.password ");
		...
		login.login(database, Login.USER_ADMIN_NAME, "admin"); // FIXME hardcoded reference to admin password hash
		...
		MolgenisUser userResearcher = createUser(database, "researcher", "Researcher", "Researcher", "molgenis@gmail.com", researchPassword, false);		
		MolgenisUser userDataManager = createUser(database, "datamanager", "DataManager", "DataManager", "molgenis@gmail.com", dataManagerPassword, false);
		...
		if("website".equals(appProfile)){
			...
			throw new RuntimeException("please configure app.profile in your molgenis-server.properties. possible values: workspace or website");
		}
	...
	private void createPermission(Database database, Class<?> clazz, MolgenisRole role, String permissionString) throws DatabaseException
	{
