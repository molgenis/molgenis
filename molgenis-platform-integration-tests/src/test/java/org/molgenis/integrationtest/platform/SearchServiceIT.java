...
public class SearchServiceIT extends AbstractTestNGSpringContextTests
{
	...
	public void testFuzzyMatch()
	{
		...
		List<Object> ids = searchService.searchAsStream(entityTypeDynamic, query).map(Entity::getIdValue)
				.collect(toList());
	...
	public void testSemanticSearch()
	{
		...
		List<Object> matchingAttributeIDs = searchService.searchAsStream(entityTypeDynamic, query)
	...
	{
		Iterable<Entity> retrieved = searchService.search(entityTypeDynamic, new QueryImpl<>());
	...
	public void testFindAllStreaming()
	{
		...
		Supplier<Stream<Entity>> retrieved = () -> searchService.searchAsStream(entityTypeDynamic, new QueryImpl<>());
	...
	public void testFindQueryOperatorEq(String attrName, Object value, List<Integer> expectedEntityIds)
	{
		...
		List<Object> ids = searchService.searchAsStream(entityTypeDynamic, query).map(Entity::getIdValue).collect(
				toList());
	...
	public void testFindQueryOperatorIn(List<String> ids, List<Integer> expectedEntityIds)
	{
		...
		List<Object> foundIds = searchService.searchAsStream(entityTypeDynamic, query).map(Entity::getIdValue).collect(
				toList());
	...
	public void testFindQueryOperatorRange(int low, int high, List<Integer> expectedEntityIDs)
	{
		...
		List<Object> foundAsList = searchService.searchAsStream(entityTypeDynamic, nestedQuery).map(
	...
	public void testSearchQueryLimit2_Offset2_sortOnInt()
	{
		...
		Iterable<Entity> result = searchService.search(entityTypeDynamic, query);
	...
	public void testFindOneQuery()
	{
		...
		entity = searchService.findOne(entityTypeDynamic, new QueryImpl<>().eq(ATTR_ID, entity.getIdValue()));
