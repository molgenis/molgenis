...
public class SearchServiceIT extends AbstractTestNGSpringContextTests
{
	...
	public void testFuzzyMatch()
	{
		...
		List<Object> ids = searchService.search(entityTypeDynamic, query).map(Entity::getIdValue)
				.collect(toList());
	...
	public void testSemanticSearch()
	{
		...
		List<Object> matchingAttributeIDs = searchService.search(entityTypeDynamic, query)
	...
	public void testFindAllStreaming()
	{
		...
		Supplier<Stream<Entity>> retrieved = () -> searchService.search(entityTypeDynamic, new QueryImpl<>());
	...
	public void testFindQueryOperatorEq(String attrName, Object value, List<Integer> expectedEntityIds)
	{
		...
		List<Object> ids = searchService.search(entityTypeDynamic, query).map(Entity::getIdValue).collect(toList());
	...
	public void testFindQueryOperatorIn(List<String> ids, List<Integer> expectedEntityIds)
	{
		...
		List<Object> foundIds = searchService.search(entityTypeDynamic, query).map(Entity::getIdValue)
				.collect(toList());
	...
	public void testFindQueryOperatorRange(int low, int high, List<Integer> expectedEntityIDs)
	{
		...
		List<Object> foundAsList = searchService.search(entityTypeDynamic, nestedQuery).map(Entity::getIdValue)
				.collect(toList());
