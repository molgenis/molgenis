...
{
	@Mock
	RepositoryAnnotator annotator1;
	@Mock
	RepositoryAnnotator annotator2;
	@Mock
	RepositoryAnnotator annotator3;
	@Mock
	RepositoryAnnotator annotator4;
	@Mock
	RepositoryAnnotator annotator5;
    @Mock
    AnnotatorInfo annotator1info;
    @Mock
    AnnotatorInfo annotator2info;
    @Mock
    AnnotatorInfo annotator3info;
    @Mock
    AnnotatorInfo annotator4info;
    @Mock
    AnnotatorInfo annotator5info;
	...
	public void beforeMethod()
	{
        ...
        when(annotator1.getInputMetaData()).thenReturn(Arrays.asList(attrd,attrh,attri));
        when(annotator1info.getOutputAttributes()).thenReturn(Arrays.asList(attre,attrc));

        when(annotator2.getInputMetaData()).thenReturn(Arrays.asList(attra,attrb));
        when(annotator2info.getOutputAttributes()).thenReturn(Arrays.asList(attrj));

        when(annotator3.getInputMetaData()).thenReturn(Arrays.asList(attra,attrh));
        when(annotator3info.getOutputAttributes()).thenReturn(Arrays.asList(attrd,attrj));

        when(annotator4.getInputMetaData()).thenReturn(Arrays.asList(attra,attrb));
        when(annotator4info.getOutputAttributes()).thenReturn(Arrays.asList(attrh));

        when(annotator5.getInputMetaData()).thenReturn(Arrays.asList(attrb,attrd));
	...
	public void testSucces()
	{
		...
		Queue<RepositoryAnnotator> result = resolver.getAnnotatorSelectionDependencyList(
                available,
                requested,
                repo);
