...
public class CaddAnnotator
{
	...
	public RepositoryAnnotator cadd()
	{
		...
		AttributeMetaData cadd_abs = attributeMetaDataFactory.create().setName(CADD_ABS).setDataType(STRING)
				.setDescription(
		...
		AttributeMetaData cadd_scaled = attributeMetaDataFactory.create().setName(CADD_SCALED).setDataType(STRING)
				.setDescription(
		...
		AnnotatorInfo caddInfo = AnnotatorInfo.create(AnnotatorInfo.Status.READY, AnnotatorInfo.Type.PATHOGENICITY_ESTIMATE, NAME,
				...
				attributes);
		EntityAnnotator entityAnnotator = new AnnotatorImpl(CADD_TABIX_RESOURCE, caddInfo, new LocusQueryCreator(vcfAttributes),
	...
	Resource caddResource()
	{
		...
		EntityMetaData repoMetaData = entityMetaDataFactory.create()
				.setName(CADD_TABIX_RESOURCE);
