...
public class CreateOneGenomeReport
{
	
	private Repository vcfRepo;
	
	public CreateOneGenomeReport(Repository vcfRepo)
	{
		this.vcfRepo = vcfRepo;
	}

	public CreateOneGenomeReport(File inputVcfFile) throws IOException
	{
		this.vcfRepo = new VcfRepository(inputVcfFile, this.getClass().getName());
	}
	
	
	public OneGenomeReport createReport() throws IOException
	{
		OneGenomeReport report = new OneGenomeReport();
		...
		/**
		 * Then, remove candidates that are EXCLUDED_FIRST_OF_COMPOUND but have no further INCLUDED_RECESSIVE_COMPOUND within the same gene
		 * Assumes exactly 1x EXCLUDED_FIRST_OF_COMPOUND
		 */
		...
		for(String gene : allCandidates.keySet())
		{
			...
			{
				String candStatus = variant.getString(MonogenicDiseaseCandidatesServiceAnnotator.MONOGENICDISEASECANDIDATE);
				if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.EXCLUDED_FIRST_OF_COMPOUND.toString()) || candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.EXCLUDED_FIRST_OF_COMPOUND_HIGHIMPACT.toString()))
				{
					variantFor_EXCLUDED_FIRST_OF_COMPOUND = variant;
				}
				else if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.INCLUDED_RECESSIVE_COMPOUND.toString()) || candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.INCLUDED_RECESSIVE_COMPOUND_HIGHIMPACT.toString()))
				{
		...
		for(String gene : allCandidates.keySet())
		{
			 ...
			 * Phenotype match
			 */
			Double phenomizerScore = oneVariant.getDouble(PhenomizerServiceAnnotator.PHENOMIZERPVAL) != null ? oneVariant.getDouble(PhenomizerServiceAnnotator.PHENOMIZERPVAL) : null;
			...
			for(Entity variant : allCandidates.get(gene))
			{
				Double caddScore = variant.getDouble(CaddServiceAnnotator.CADD_SCALED)  != null ? variant.getDouble(CaddServiceAnnotator.CADD_SCALED) : null;
				if(caddScore != null)
				{
					if(caddScore > 30)
					{
			...
			for(Entity variant : allCandidates.get(gene))
			{
				...
				else if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.INCLUDED_RECESSIVE.toString()))
				{
					stars++;
				}
				
				
				else if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.INCLUDED_DOMINANT_HIGHIMPACT.toString()))
				{
					stars += 2;
				}
				else if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.INCLUDED_RECESSIVE_HIGHIMPACT.toString()))
				{
					stars += 2;
				}
				
				
				else if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.INCLUDED_RECESSIVE_COMPOUND.toString()))
				{
					stars++;
				}
				else if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.INCLUDED_RECESSIVE_COMPOUND_HIGHIMPACT.toString()))
				{
					stars += 2;
				}
				
				//some special cases: add a little extra weight on recessive compounds of which the FIRST one was high impact
				else if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.EXCLUDED_FIRST_OF_COMPOUND_HIGHIMPACT.toString()))
				{
					stars++;
				}
				//and also add a star for 'OTHER' inheritance
				else if(candStatus.equals(MonogenicDiseaseCandidatesServiceAnnotator.outcome.INCLUDED_OTHER.toString()))
				{
					stars++;
				}
	...
	public static void main(String[] args) throws Exception
	{
		...
		CreateOneGenomeReport cr = new CreateOneGenomeReport(inputVcfFile);
		cr.createReport();
	}
}
